    ...
    e.AmSerialChart = e.Class({
        ...
        countColumns: function() {
            ...
            for (k = 0; k < b; k++) {
                ...
                var l = g.stackType;
                if ("100%" == l || "regular" == l)
                    for (e = !1, f = 0; f < c; f++) d = this.graphs[f], d.tcc = 1, d.valueAxis ==
    ...
    e.Cuboid = e.Class({
        ...
        draw: function() {
            ...
            if (0 < Math.abs(h) || 0 < Math.abs(f))
                if (isNaN(r)) F = v, v = e.adjustLuminosity(C, -.2), v = e.adjustLuminosity(C, -.2), w = e.polygon(b, [0, h, d + h, d, 0], [0, f, f, 0, 0],
            ...
            d = isNaN(r) ? 0 > g ? [w,
                M, A, z, G, y, D, F, L, b
            ] : [F, L, A, z, G, y, w, M, D, b] : K ? 0 < d ? [w, b, F] : [F, b, w] : 0 > g ? [w, b, F] : [F, b, w];
            e.setCN(c, b, t + "front");
            e.setCN(c, A, t + "back");
            e.setCN(c, F, t + "top");
            ...
            if (e.ifArray(a)) {
                ...
                if (this.parseDates && !this.equalSpacing) this.lastTime = a[a.length - 1].time, this.maxTime = e.resetDateToMin(new Date(this.lastTime + 1.05 * x), this.minPeriod,
                    ...
                    if (this.cellWidth = this.getStepWidth(f), f < k && (k = f), h += this.start, this.stepWidth = this.getStepWidth(f), 0 < k)
                        for (k = Math.floor(f / k), f = this.chooseMinorFrequency(k), p = h, p / 2 == Math.round(p / 2) && p--, 0 > p && (p = 0), v = 0, this.widthField && (p = this.start), this.end - p + 1 >= this.autoRotateCount && (this.labelRotationR = this.autoRotateAngle), b = p; b <= this.end + 2; b++) {
                            n = !1;
                            0 <= b && b < this.data.length ? (u = this.data[b], p = u.category,
                                n = u.forceShow) : p = "";
                            if (t && !isNaN(f))
                                if (b / f == Math.round(b / f) || n) b / k == Math.round(b / k) || n || (this.gridAlpha = this.minorGridAlpha, p = void 0);
                                else continue;
                    ...
                    a = p;
                    if (this.data[p].time != e.resetDateToMin(new Date(this.data[p].time), g, u, q).getTime())
                        for (q = 0, z = x, b = p; b < A; b++) f = this.data[b].time, this.checkPeriodChange(g, u, f, z) && (q++, 2 <= q && (a = b, b = A), z = f);
                    t && 1 < u && (f = this.chooseMinorFrequency(u), e.getPeriodDuration(g,
                        f));
                    if (0 < this.gridCountR)
                        for (b = p; b < A; b++)
                            if (f = this.data[b].time, this.checkPeriodChange(g, u, f, x) && b >= a) {
                ...
                for (b = l = 0; b < this.data.length; b++)
                    if (v = this.data[b]) this.parseDates && !this.equalSpacing ? (m = v.time, d = this.cellWidth, "MM" == this.minPeriod && (d = 864E5 * e.daysInMonth(new Date(m)) * this.stepWidth, v.cellWidth = d), m = Math.round((m - this.startTime) * this.stepWidth + d / 2)) :
                        m = this.getCoordinate(b - h), v.x[this.id] = m;
                if (this.widthField)
                    for (b = this.start; b <= this.end; b++) v = this.data[b], d = v.widthValue, v.percentWidthValue = d / w * 100, this.rotate ? (m = this.height * d / w / 2 + l, l = this.height * d / w + l) : (m = this.width * d / w / 2 + l, l = this.width * d / w + l), v.x[this.id] = m;
